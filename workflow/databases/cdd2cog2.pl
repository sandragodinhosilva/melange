...
C<perl cdd2cog.pl -r rps-blast.out -c cddid.tbl -f fun.txt -w whog -o outdir>
...
my $VERSION = 0.1;
my $Out_Dir; #path to the output directory
...
die "$0 $VERSION\n" if ($Opt_Version);
if (!$Rps_Report || !$CDDid_File || !$Fun_File || !$Whog_File || !Out_Dir) {
    my $warning = "\n### Fatal error: Option(s) or arguments for '-r', '-c', '-f', '-w' or 'o' are missing!\n";

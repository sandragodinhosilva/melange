...
""".replace(
    ...
    def test_degree(self):
        ...
        assert_equal(sorted(A.iterdegree(A.nodes())), [("1", 1), ("2", 2), ("3", 1)])
    ...
    def test_graph_not_strict(self):
        ...
        A.add_edge("3", "3", "foo")
        assert_equal(A.edges(), [("1", "2"), ("1", "2"), ("3", "3"), ("3", "3")])
        assert_equal(
            A.edges(keys=True),
...
class TestDiGraphOnly(TestGraph):
    ...
    def test_edges(self):
        ...
        assert_equal(sorted(A.out_degree(with_labels=True).values()), [0, 1, 1])
        assert_equal(sorted(A.out_degree_iter()), [("1", 1), ("2", 1), ("3", 0)])
        assert_equal(sorted(A.in_degree()), [0, 1, 1])
        assert_equal(sorted(A.in_degree_iter()), [("1", 0), ("2", 1), ("3", 1)])

...
def FilesToUse():
    ...
    extensionsToCheck = (ko_pattern, pfam_pattern, cog_pattern, cazyme_pattern)
    for filename in entries:
...
def GetCounter(dataset, d, df_stats=df_stats):
    ...
    df_stats = df_stats[["index", "orfs"]]
    merge = pd.merge(df_counter_abund, df_stats, how="left", left_on="index", right_on="index").set_index("index")
    merge = merge.apply(pd.to_numeric, errors='ignore')
    numeric_cols = merge.select_dtypes(exclude=["object"]).columns.to_list()
    numeric_cols.remove("orfs")
    merge[numeric_cols] = merge[numeric_cols].div(merge["orfs"], axis=0).multiply(100)
    df_counter_abund = merge.copy()
    df_counter_abund = df_counter_abund.T.drop(index=["orfs"]).reset_index()
...
###############################################################################
## Pfams
df_counter_pfam, df_counter_pfam_PA,df_counter_pfam_abund = GetCounter("Pfam", d_count_pfam)
